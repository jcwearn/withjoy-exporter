...
def test_configured_requires_all_three():
    with patch.object(github_sync, "APP_ID", "1"), \
...
def test_app_jwt_claims():
    with patch.object(github_sync, "APP_ID", "123456"), \
...
def test_find_run_tolerates_clock_skew():
    ...
    with patch.object(github_sync, "_runs", return_value=[_run(run_id=4, created="2026-07-01T11:59:58Z")]):
...
def test_summarize_run_maps_github_states():
    ...
    assert github_sync.summarize_run(
